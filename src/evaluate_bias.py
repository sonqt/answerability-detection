...
def evaluate(dataset, predictions, analysis):
    ...
    for key in total:
        em_mean = statistics.stdev(exact_match[key])
        em_std = statistics.mean(exact_match[key])
        exact_match[key] = [em_mean, em_std]
        f1_mean = statistics.stdev(f1[key])
        f1_std = statistics.mean(f1[key])
        f1[key] = [f1_mean, f1_std]
